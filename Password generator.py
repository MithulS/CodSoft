...
class Window():
    ...
    MAX_CHARS = 16
    MIN_CHARS = 3
    CHARS_OPTIONS = ["Alphanumeric (m12it1292)", 
                    "Numeric (9348332)", 
                    "Alpha (MitHulwwwwW)"]
    GRID_PADY = (18, 18)
    def __init__(self):
    ...
    def initUI(self):
        ...
        self.text_password_out = tkinter.Text(self.master, border=2, height=2, width=30,bg='red',fg='white')
        
        self.frame_buttons = tkinter.Frame()
        self.button_generate = tkinter.Button(self.frame_buttons, text="Generate", width=8, command=lambda: self.set_password(),bg = 'red',fg='white',activebackground='black')
        self.button_close = tkinter.Button(self.frame_buttons, text="Close", command=self.master.quit, width=8,bg = 'black',fg='white',activebackground='red')
